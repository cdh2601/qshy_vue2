<template>
  <div class="commentBox">
    ...
    <h3>最新评论</h3>
    <div>
      <CommentView ref="commentRef" :parentId="$props.articleId" type="Strategy"></CommentView>
    </div>
  </div>
</template>
...
export default {
      ...
      if (!this.tokenStr) {
        ...
        if (this.commentContent !== "") {
          const params = {
            ...
            // 父评论  没有为0
            "strategyId": this.articleId
          }
          Axios.post("/comment/saveStrategyComment",
              {}, {params}
          ).then(res => {
            if (parentId == 0) {
              // 正常评论
  ...
  methods: {
    loadData() {
      ...
      Axios.get("/comment/getStrategyComments", {
        params
      }).then(res => {
        // console.log(res.data);
        this.loading = false
        this.state.commentInfo = res.data.comments;
      })
    }
  },
  mounted() {
