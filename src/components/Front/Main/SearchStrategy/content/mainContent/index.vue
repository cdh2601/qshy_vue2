<template>
  <div class="boutique_article">
    <div class="titleBox">
      <p>搜索结果</p>
      <h3># {{ state.keyword }}</h3>
      <div class="moreInfo">为您查询到<b class="ml5 mr5">{{ state.articleCount }}</b>条</div>
    </div>
    <div class="contentBox" v-loading="state.loading" element-loading-text="玩命加载中..." style="z-index:0">
      <div class="titleWrapper">
        <h3 class="title"><b>全部</b></h3>
      </div>
      <div v-if="state.articleInfo.length < 1" class="emptyImg"><p style="padding-top: 270px;">暂无相关数据</p></div>
      <div
          class="articleBox"
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      const params = {
        ...
        strategyName:this.state.search
      }
      Axios.get("/strategy/search", {
        ...
        this.state.articleCount = res.data.strategies.total;
  ...
  mounted() {
    this.state.search= this.$route.query.keyword
    this.loadData()
  ...
  updated() {
    if (this.state.keyword !== '') {
      ...
      nodes.forEach(item => {
        ...
        str = str.split(this.state.keyword.toLocaleLowerCase()).join(
            "<font style='color:#f73131;'>" + this.state.keyword.toLocaleLowerCase() +
            "</font>")
        item.innerHTML = str
      })
    }
  ...
  // watch: {
  //   this.$route.query.keyword(newValue, oldValue)
  //   {
  //     this.$router.go(0)
  //   }
  // }
