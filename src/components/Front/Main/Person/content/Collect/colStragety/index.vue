    ...
    >
      <div class="titleWrapper">
        ...
        <!-- <button @click="goWrite"><i class="el-icon-edit"></i>写攻略</button> -->
                <div class="search">
          <input type="text" v-model="keyWordStrategy" placeholder="攻略名称"/>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      Axios.get("/strategy/getUserCollectionStrategies", {
        params,
      }).then((res) => {
        // console.log(res);
        // 先清空数据
    ...
    searchRoutes(resetPage){
      ...
      // }
      // console.log(this.state.articleInfo)
      Axios.get('/strategy/searchCollection',{
        params:{
          strategyName: this.keyWordStrategy,
          pageNum: resetPage==undefined?this.state.currentPage:1,
          pageSize: this.state.pageSize,
          userId: JSON.parse(sessionStorage.getItem('userInfo')).userId,
          // userId: this.state.articleInfo.userId,
        }
      }).then((res)=>{
        // console.log(res)
        this.state.articleInfo = [];
