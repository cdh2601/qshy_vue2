...
<script>
import recRouteItem from '@/components/Front/Main/Recommend/ContentAll/recRoute/recRouteItem'
  ...
  methods:{
    searchRoutes(){
      ...
      if(this.keyWordRoute===''){
        ...
        //   return v.routeName.includes(this.keyWordRoute)
        // })
        this.recRoutes=this.recRoutes.filter(v=>{
        ...
        //   for(let j=0;j<this.recRoutes[i].scenics.length;j++){
        //
        //   }
        // }
      }
    ...
    getRecRoutes(){
      ...
      Axios.get('/route/getUserCollectionRoutes',{
        params
      }).then((res)=>{
        // console.log(res)
        // this.recRoutes=res.data.routes;
