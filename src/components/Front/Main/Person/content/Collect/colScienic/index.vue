<template>
  <div class="container">
    ...
    <div class="recommend">
      ...
      <div class="search">
        <input type="text" v-model="keyWordScenic" placeholder="景点名称"/>
        <button type="button" class="searchbtn" @click="searchScenics(1)">
          <div>
...
export default {
  ...
  methods: {
    searchScenics(resetPage){
      if(!this.keyWordScenic){
        this.getAttractions(0,null)
      }else{
        Axios.get('/scenic/searchCollection', {
          params: {
            userId: JSON.parse(sessionStorage.getItem('userInfo')).userId,
            pageNum:  resetPage==undefined?this.pageNo:1,
            pageSize: this.pageSize,
            type: this.type, //为了传值
            search:this.keyWordScenic
          }
        }).then((res)=> {
          // console.log(res)
      ...
      // if(this.keyWordScenic===''){
      //   this.$message('请输入景点名称')
      //   return
      // }
      // this.getAttractions()
    },
    getAttractions(index, type) {
      this.keyWordScenic=''
      ...
      Axios.get("/scenic/getUserCollectionScenics", {params}).then((res) => {
        // console.log(res);
        this.total = res.data.scenics.total;
        this.attractions = res.data.scenics.records;
      ...
      //index判断栏目有没有变化，type用来传类型给后端，两个为了
      // if (this.keyWordScenic == '') {
      //   if (index != this.isChange) { //index判断是否变化，如果变了，颜色变化，页码变为1
      //     this.isChange = index;
      //     this.pageNo = 1
    ...
    getPageNo(pageNo) {
      this.pageNo = pageNo;
      // console.log(this.keyWordScenic)
      if(this.keyWordScenic){
        this.searchScenics()
      }else{
